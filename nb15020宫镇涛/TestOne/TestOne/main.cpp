...
//  OpenGLOne
//
//  Created by gongzhentao on 15/12/9.
...
void display(void)
{
    ...
    /*绘画太阳*/
    glPushMatrix();//将矩阵压入栈中
    glColor3f(1.0, 0.0, 0.0);
    glRotatef((GLfloat)year, 0.0, 1.0, 0.0);//太阳自转，参数：角度，x，y，z
    glutWireSphere(1.01, 20, 16);//渲染一个球体，参数半径，经度条数，纬度条数
    glPopMatrix();
    
    /*绘制行星*/
    glPushMatrix();
    glRotatef((GLfloat)year, 0.0, 1.0, 0.0);//行星公转
    glTranslatef(3.0, 0.0, 0.0);//将图像向xyz轴平移
    ...
    glRotatef(90.0f, 1.0, 0.0, 0.0);
    glutSolidTorus(0.005, 3.0, 10, 64);//粗细,半径,
    glPopMatrix();
...
int main(int argc, char * argv[]) {
    ...
    glutReshapeFunc(reshape);//窗口大小改变时重绘
    //glutRotateFunc(rotate);
    glutIdleFunc(idle);//周期函数
    glutMainLoop();
    return 0;
}